...
The style values live in root `presets/captions/ass_caption_styles.json` so
...
DEFAULT_STYLE_CONFIG = REPO_ROOT / "presets" / "captions" / "ass_caption_styles.json"
...
def collect_impact_words(edl: dict[str, Any], config: dict[str, Any], style: dict[str, Any]) -> set[str]:
    words: set[str] = set()
...
def format_chunk_text(
    ...
    secondary = ass_color(str(style.get("secondary_accent_color", style.get("accent_color", "#F4C95D"))))
    rendered: list[str] = []
    for word in chunk:
...
def orientation_value(style: dict[str, Any], base_key: str, portrait: bool, default: float) -> float:
...
def ass_style_line(
    ...
    font_size = max(16, round(height * orientation_value(style, "font_size_ratio", portrait, 0.044)))
...
def build_master_ass(
    ...
    for chunk in chunks:
        ...
        text = format_chunk_text(chunk, style, config, impact_words, word_replacements, text_replacements)
        if not text:
            continue
