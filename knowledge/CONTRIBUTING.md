...
Add the tutorial to `knowledge/research/catalogs/youtube_tutorial_list.md` with:
...
Place raw transcript text in `knowledge/research/transcripts/` and curated,
paraphrased source notes in `knowledge/research/source_notes/`.
...
Create a lesson file in `knowledge/research/lessons/`.
...
Copy `knowledge/techniques/_template.json` and fill every relevant field.
...
Patch the relevant files in `knowledge/playbooks/`.
...
If the lesson implies repeatable settings, add a preset suggestion under `knowledge/presets/`.
...
If the tutorial warns about a failure mode, add it to `knowledge/quality/editorial_checklists/`.
