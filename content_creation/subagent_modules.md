...
| Knowledge compiler / creative director | `knowledge_plan.json`, style pack, selected skills/cards/presets/QC | `creative_directive.md`, `creative_directive.json` with caption/motion/sound/color/layout contracts | `knowledge_compiler.py` |
| Scriptwriter | `script_brief.md`, topic, style pack, research notes, relevant skills | `script_drafts/round_XX.md` | Dedicated writer subagent using `skills/scriptwriting.md`, `content_creation/scriptwriter.md`, and task-specific knowledge |
| Script critic | `script_brief.md`, current draft, relevant QC/story knowledge | `script_critique.md` with `PASS` or `FAIL` | Independent harsh critic subagent using `content_creation/script_subagent_review_loop.md` |
...
| Video renderer | `edit_decision_list.json`, assets, rich captions, VO | `preview.mp4`, `final.mp4` | `faceless_renderer.py` |
...
- Renderer/QC agent: runs `faceless_renderer.py`, verifies rich layers are
  visible but bounded, then runs `qc_check.py`, fixes failures, and renders
  final.
